...
PKG_NAME = f"pymixer_{VERSION}"
PKG = os.path.join(PARENT, PKG_NAME)

BIN = os.path.join(PKG, "usr", "local", "bin", "pymixer_utils")
START = os.path.join(PKG, "usr", "local", "bin", "pymixer")
...
cmd = "python3.8 /usr/local/bin/pymixer_utils/main.py "
for a in sys.argv[1:]:
    cmd += a
    cmd += " "
os.system(cmd.strip())
...
while len(dirs) > 0:
    for f in os.listdir(os.path.join(SRC, d:=dirs.pop())):
        ...
        if os.path.isfile(abspath) and ".py" in abspath:
